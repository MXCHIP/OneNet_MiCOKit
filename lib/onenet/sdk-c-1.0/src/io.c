...
{
    struct avro_reader_t_ *mem_reader = (struct avro_reader_t_ *) regina_calloc(1,
...
{
    struct avro_writer_t_ *mem_writer = (struct avro_writer_t_ *) regina_calloc(1,
...
static int avro_read_memory(struct avro_reader_t_ *reader, void *buf, int64_t len)
{
    if (len > 0) {
        ...
        regina_memcpy(buf, reader->buf + reader->read, len);
        reader->read += len;
    }
    return 0;
}
...
static int avro_write_memory(struct avro_writer_t_ *writer, void *buf, int64_t len)
{
    if (len) {
        ...
        regina_memcpy((void *) (writer->buf + writer->written), buf, len);
        writer->written += len;
    }
    return 0;
}
