...
void ESP8266_Init(int8_t * server, int8_t * ssid_pwd)
{
    ...
    SendCmd(ssid_pwd, "CONNECTED", 2000);  /*配置要连接的路由器SSID和密码*/
	#if 0	
  //  SendCmd(server, "OK", 2000); /*与服务器建立TCP连接*/
    SendCmd(CIPMODE, "OK", 1000); /*透传模式*/

	 SendCmd(CIPSEND, "OK", 1000);

    memset(usart2_rcv_buf, 0, sizeof(usart2_rcv_buf)); /*先清除接收缓冲区*/
    usart2_rcv_len = 0;
...
int connect_tcp(char *addr,int port)
{
	char server[64];
	int ret=-1,n=10;
	snprintf(server,sizeof(server),"AT+CIPSTART=\"TCP\",\"%s\",%d\r\n",addr,port);
    ret=SendCmd("+++", "+++", 30); 
	
	ret=SendCmd(AT, "OK", 300); /*进入AT命令模式*/
	if(ret<0)
	{
		return -1;
	}
	ret=SendCmd(server, "OK",200); /*与服务器建立TCP连接*/
	if(ret==3)
	{
		ret=SendCmd("AT+CIPCLOSE\r\n", "OK",200);
		if(ret<0)
		{
			return -1;
		}
		mDelay(2000);
		ret=SendCmd(server, "CONNECTED",200);
		if(ret<0)
		{
			return -1;
		}
	...
	ret=SendCmd(CIPMODE, "OK",200); /*透传模式*/
	if(ret<0)
	{
		return -1;
	}
	
	ret= SendCmd(CIPSEND, "OK", 1000);
		if(ret<0)
	{
		return -1;
	}
...
uint32_t USART2_RecvData(int8_t * buf, uint32_t len)
{
    int len_in=USART2_GetRcvNum();
	  if(len_in==0)
		{
			return len_in;
		}
